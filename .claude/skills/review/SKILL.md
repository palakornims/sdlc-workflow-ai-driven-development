...
# /review — Phase 5: Code Review (first pass)
...
3. Invoke the `code-reviewer` subagent (`subagent_type: "code-reviewer"`).
...
5. **Human-only Codex commands.** Tell the user, verbatim from the reviewer's instructions:
   ...
   then check `/codex:result` and paste the output into `/triage $ARGUMENTS`. You must not run
   `/codex:review`, `/codex:adversarial-review`, `/codex:status`, or `/codex:result` yourself.
6. Remind the user to record their Codex run with `/log` (the logging rule applies to humans).
