...
4. **Next command** for each item: one of `/plan`, `/design`, `/breakdown`,
   `/implement TASK-NNN`, `/review TASK-NNN`, `/triage TASK-NNN`, `/test STORY-NNN`,
   `/validate STORY-NNN`, or a `/log` the human still owes.
