...
2. Invoke the `product-owner` subagent (`subagent_type: "product-owner"`).
...
4. Route findings: built wrong → `/triage <TASK>` with the DEF-; built the wrong thing →
   `/plan PRD-NNN` to revise the requirement; scope creep → ask the user to decide.
5. Ask the human product owner to sign off with `/log`, or name the phase to return to.
