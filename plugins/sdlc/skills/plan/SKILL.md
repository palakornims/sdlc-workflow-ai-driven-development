...
yourself; the `sdlc:product-owner` agent does.
...
1. Invoke the `sdlc:product-owner` subagent with the Agent tool (`subagent_type: "sdlc:product-owner"`).
...
4. End with the approval reminder: the PRD and stories must be reviewed and marked Approved
   (approver and date in the document header) before `/sdlc:design` may run. Offer `/sdlc:log` to record
   the approval.
