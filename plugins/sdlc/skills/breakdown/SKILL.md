...
# /sdlc:breakdown — Phase 3: Breakdown
...
2. Invoke the `sdlc:project-manager` subagent (`subagent_type: "sdlc:project-manager"`).
...
3. If the PM sends design elements back, relay them and suggest `/sdlc:design` to revise.
4. Relay the final report: milestones, task count, critical path, human-input tasks, gate.
5. Remind: the backlog must be Approved before `/sdlc:implement`. Offer `/sdlc:log`.
