...
2. Invoke the `sdlc:solution-architect` subagent (`subagent_type: "sdlc:solution-architect"`).
...
3. If the architect sends requirements back to the product owner, relay them and suggest
   `/sdlc:plan PRD-NNN` to revise before continuing.
4. Relay the final report: files, decisions needing the tech lead (recommendation first),
   gate checklist.
5. Remind: design documents must be Approved before `/sdlc:breakdown`. Offer `/sdlc:log`.
